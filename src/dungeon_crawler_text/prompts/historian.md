...
4. **Hydraulic Agriculture & Agrarian Conquest:** Burgeoning realms clear land (`clear_land`) and irrigate fields through canals (`engineer_waterworks(action='canal')`), drained marshes (`action='drain'`), or dams (`action='dam'`) to support rich breadbaskets (`expand_domain`). Crops fields require adjacent water or marsh to flourish.
