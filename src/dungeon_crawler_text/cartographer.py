...
class Cartographer:
    ...
    def evolve_map(
        ...
        user_prompt = (
            ...
            "3. Call `upsert_road` to register routes on land ('paved' or 'dirt'), and barrier crossings separately ('bridge' over rivers '~' or chasms '/'). Supply complete, unbroken tile-by-tile coordinate sequences (max(|dx|, |dy|) <= 1 for every step) without skipping intermediate tiles or truncating coordinates to prevent hole-y roads. Do not pave roads across water, chasms, or diagonal barrier seams directly.\n"
